...
In this tutorial we will learn how to analyze network data in R to measure complex metrics. As in the introduction tutorial about how to handle network data in R, we are going to use as an example the social network of interactions between the characters of [Star Wars Episode IV](http://evelinag.com/blog/2015/12-15-star-wars-social-network/).
...
Through tidygraph, you can also calculate the k-core decomposition of the network. It is as simple as using the node_coreness() function in our pipeline:
```{r }
starwars %>% 
  activate(nodes) %>% 
