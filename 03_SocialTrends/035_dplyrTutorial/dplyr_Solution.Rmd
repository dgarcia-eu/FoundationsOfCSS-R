...
or a range:
```{r select3}
select(NHIS2007, SEX:educ) 
```

Your turn: select the first and last columns.
```{r select4}
select(NHIS2007, c(1,ncol(NHIS2007))) 
```
...
```{r filter1}
filter(NHIS2007, SLEEP < 8)
```

Your turn: get the rows with BMI between 20 and 30
```{r filter2}
filter(NHIS2007, BMI > 20 & BMI < 30)
```
...
df2 <- data.frame(a=c(2,3,4), x=c("x","y","z"), z=c(1,1,1))
inner_join(df1,df2)
```

inner_join automatically takes the common columns for the merging, but you can also specify which columns to use:
```{r merge2}
inner_join(df1,df2, by=c("a"="z"))
```

For further practice at home: Take your solution of our FOI and GDP exercise and change the merge commands to inner_join commands.
